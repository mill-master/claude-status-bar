...
- **The permission dot pulses when a wait drags (Linux).** After half a minute unanswered, the amber dot alternates with a faded one about once a second, so a stuck permission catches the eye that a static dot slips past. It stops the moment the session moves.
- **Click a session row to jump to its terminal (Linux).** The row raises the terminal hosting that session (the app, not the exact tab, matching macOS). On GNOME the raise runs through a bundled shell extension, since Wayland refuses focus moves from background apps; the app enables it automatically on first launch and GNOME activates it at the next login (a click before then explains itself with a one-time notification). An explicit `gnome-extensions disable` is respected and never overridden. Sway/Hyprland use the compositor's focus command, X11 uses wmctrl, VS Code sessions raise VS Code. Rows with no route (ssh, unknown emulators) stay display-only.
