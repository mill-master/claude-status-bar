...
- **Click a session row to jump to its terminal (Linux).** The row raises the terminal hosting that session (the app, not the exact tab, matching macOS): DBus activation for GNOME Terminal, Ptyxis, Console and Tilix on any desktop, the compositor's own focus command on Sway/Hyprland, wmctrl on X11, and `code` for VS Code sessions. Rows with no route (ssh, unknown emulators) stay display-only.
