...
- **Desktop notifications (Linux, opt-in).** A session entering permission-wait, or a turn finishing after a minute or more, can raise a desktop notification named by project, reaching you where the bar cannot (fullscreen apps, other workspaces). New Notifications menu: Off (default) / Permission / Turn end / Permission + turn end. Turn end alone is the mode for bypass-permissions and auto-accept workflows, which never see a permission prompt but do fire long unattended turns.
