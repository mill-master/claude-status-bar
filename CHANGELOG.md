...
- **Desktop notifications (Linux, opt-in).** A session entering permission-wait, or a turn finishing after a minute or more, can raise a desktop notification named by project, reaching you where the bar cannot (fullscreen apps, other workspaces). New Notifications menu: Off (default) / Permission / Permission + turn end.
