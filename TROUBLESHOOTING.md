...
**Clicking a session row: what it raises, and the one-time GNOME step.** A row click brings that session's terminal APP to the front (the app, not the exact tab, same as macOS). The terminal is found by walking the session's process ancestry. On GNOME, Wayland forbids background apps from moving focus, so the raise goes through the bundled shell extension; enable it once with `gnome-extensions enable claude-status-bar-focus@mill-master.github.io` (after the first install, log out and back in so GNOME sees the new extension). Without it, rows are display-only. Sway/Hyprland use the compositor's focus command, plain X11 uses wmctrl, VS Code sessions raise VS Code via `code`. Sessions with no route (over ssh, an emulator the app doesn't know) stay display-only; each click logs its route to `~/.cache/claude-status-bar/focus.log`.
...
**No turn-end popup?** The "Turn-end notification" check item only appears when libnotify is present (`sudo apt install gir1.2-notify-0.7`), and popups also need a notification daemon: desktop environments run one, minimal Sway/Hyprland setups add one (mako, dunst). Off by default; turn it on from the icon's menu, and turns shorter than a minute stay quiet.
