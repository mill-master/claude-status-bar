...
**Clicking a session row: what it raises.** A row click brings that session's terminal APP to the front (the app, not the exact tab, same as macOS). The terminal is found by walking the session's process ancestry. On GNOME the raise goes through a bundled shell extension (Wayland forbids background apps from moving focus); the app enables it for you on first launch, and GNOME picks it up at your next login. Clicking before then shows a one-time note instead of silently doing nothing. To turn it off: `gnome-extensions disable claude-status-bar-focus@mill-master.github.io` (the app never re-enables over an explicit disable). Sway/Hyprland use the compositor's focus command, plain X11 uses wmctrl, VS Code sessions raise VS Code via `code`. Sessions with no route (over ssh, an emulator the app doesn't know) stay display-only; each click logs its route to `~/.cache/claude-status-bar/focus.log`.
